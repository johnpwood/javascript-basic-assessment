...
for(key in snake){snake[key] = key.slice(0,1)==='s'?'s':snake[key];}
...
rainbowColors = "red orange yellow green blue indigo violet".split(" ");
...
secondHero = heroes.slice(heroes.indexOf('flash'),1);
...
var removeItem = (a, item) => {
	while(a.indexOf(item)>=0){
...
getValueOfProperty = (obj, prop) => obj[prop];
...
var makeChatMessage = (m, author) => {
	return {message: m, author: author, timestamp: new Date()};
};
...
var coderTest = person => {
	if(person.name === 'Jeremy'){person.lovesCode=10; return;}
	if(person.name === 'Brack'){person.lovesCode=0; return;}
	person.lovesCode = 5;
}
...
var outside = function(t, h, c){ //temperatur, humidity, cloudiness
	if(t>80 && h>40){return "I'm all sweat";}
	if(t<40 && c>60){return "I have icecicles";}
	if(t>80 && h<40 && c<20){return "I'm literally in the desert";}
	if(t>80 || h>50 || c>50){return "Hmm, probably not";}
